...
export class ViasPublicitariasService {
  ...
  validar(dato: any){
    if(this.generales.validarString(dato.nombre)){
      this.generales.mensajeError('No se ha ingresado el nombre de la via publicitaria');
      return false;
    }
    if(this.generales.validarEntero(dato.idMedioPublicitario)){
      this.generales.mensajeError('No se ha seleccionado el medio publicitario de la via publicitaria');
      return false;
    }
    return true;
  }
