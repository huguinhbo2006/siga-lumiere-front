...
export class TiposIngresosService {
  ...
  uri = environment.url+'tiposIngresos/';
  ...
  validar(dato: any){
    if(this.generales.validarString(dato.nombre)){
      this.generales.mensajeError('No se ha ingresado el nombre del tipo de ingreso');
      return false;
    }
    if(this.generales.validarEntero(dato.idRubro)){
      this.generales.mensajeError('No se ha seleccionado el rubro del tipo de ingreso');
      return false;
    }
    return true;
  }
