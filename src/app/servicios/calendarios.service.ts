...
export class CalendariosService {
  ...
  validar(dato: any){
    if(this.generales.validarString(dato.nombre)){
      this.generales.mensajeError('No se ha ingresado el nombre del calendario')
      return false;
    }
    if(this.generales.validarString(dato.inicio)){
      this.generales.mensajeError('No se ha ingresado el inicio del calendario')
      return false;
    }
    if(this.generales.validarString(dato.fin)){
      this.generales.mensajeError('No se ha ingresado el fin del calendario')
      return false;
    }
    return true;
  }
