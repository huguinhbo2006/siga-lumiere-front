...
export class ReporteAlumnosComponent {
  lista: any;
  headers = [
    'No.',
    'FOLIO INSC',
  ...
  traer() {
    this.cargando = true;
    this.reportes.reporteImpartidos(this.busqueda).subscribe(respuesta => {
  ...
  excel() {
    exportCSVFile(this.headers, this.datos, 'Alumnos de Plantel ' + localStorage.getItem('nombreSucursal'));
  }
}
