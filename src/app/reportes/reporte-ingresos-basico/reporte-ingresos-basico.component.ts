...
@Component({
  selector: 'app-reporte-ingresos-basico',
  templateUrl: './reporte-ingresos-basico.component.html',
  styleUrl: './reporte-ingresos-basico.component.css'
})
export class ReporteIngresosBasicoComponent {
  lista: any;
  headers = [
              'No.',
              'Folio',
  ...
  traer() {
    this.cargando = true;
    this.reportes.ingresosBasico(this.busqueda).subscribe(respuesta => {
      this.cargando = false;
  ...
  excel() {
    exportCSVFile(this.headers, this.datos, 'Ingresos Básicos');
  }
}
